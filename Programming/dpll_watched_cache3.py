...
class DPLL_watched_solver:
    ...
    def unit_prop(self, literal: Optional[int] = None):
        ...
        while u_literals:
            ...
            if abs(lit) not in self.counters:
                self.counters[abs(lit)] = collections.deque(maxlen=8)

            self.counters[abs(lit)].append(lit if i > 0 else -lit)
    ...
    def solve(self) -> tuple[bool, list[int]]:
        ...
        while unassigned:
            ...
            lit = random.sample(unassigned,1)[0]

            # try lit
            if abs(lit) in self.counters:
                random.sample(self.counters[lit], 1)[0]
            else:
                lit = random.choices([lit,-lit],[.5,.5])[0]
            ...
            # try -lit
                ...
                while assigned:
                    lit, fit = assigned.pop()
                    rollback(lit)
                    # +lit was always tried first, so it can be changed
                    if (not lit == fit) and u_prop(-lit):
                ...
                if not assigned:
                    ...
                    return False, None
        self.solve_time = perf_counter_ns() - start
        return True, sorted(map(lambda x: x[0],self.assigned), key=lambda i: abs(i))
